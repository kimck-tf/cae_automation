...
| ClaudeApp | `C:\_PYTHON\0_CK_Project\17. ClaudeApp` | H-Chat 게이트웨이 경유 claude-agent-sdk 검증 경로, GUI 헤드(P-GUI)의 기반 자산(D7). **P-GUI 실장: 세션·게이팅 패턴 계승**(영속 워커 스레드+asyncio 루프+장수 ClaudeSDKClient·can_use_tool 권한 브리지), **GUI 계층은 CustomTkinter→PySide6 참고 재작성**(이식 아님). 하네스 취급은 **반전** — ClaudeApp `setting_sources=[]`·`strict_mcp_config` 격리 ↔ GUI 헤드 `["project"]` 상속(D7). default-DENY는 하네스 OFF 격리 앱의 정책(§10.2 — GUI 헤드는 미채택) |
...
- **쓰기 직렬화(Codex F2)**: 원장·GateResult 쓰기는 **job별 OS 파일락**으로 직렬화한다 — 인간 승인 채널(별도 프로세스 `pilot approve`)과 에이전트 세션 훅(자동 G1)이 같은 job에 동시 기록하는 것이 정상 운영 흐름이므로, seq 단조·attempt 유일을 락으로 보장한다. OS 락은 프로세스 종료 시 자동 해제라 stale lock이 없다. **직렬화 대상에 manifest 재생성(`rebuild_manifest`)도 포함한다(Codex 3차 #6·P-GUI Task 0)**: rebuild가 read_events→fold→atomic rename 전 구간을 락 없이 수행하면 동시 writer의 신 이벤트를 구 원장 기준 fold가 **후퇴 덮어써** 훅의 paused/게이트 검사(read_manifest)까지 오염될 수 있으므로, rebuild 본문도 `_job_lock` 임계구역으로 감싼다(비재진입 — 락 보유 문맥용 `_rebuild_manifest_unlocked` 분리, 호출자 시그니처 불변). GUI timeout 복구(ApprovalExecutor의 rebuild+재조회, §6.1)의 '확정' 의미론이 이 원자화를 전제한다.
...
- **검토 시점 결속 (whole-tree 적대 리뷰 2026-07-15 C-1)**: 보호 경로는 *누가 썼는가*만 보증할 뿐 *무엇을 검토했는가*를 보증하지 않는다 — 결과 파일이 기록 후 잔존하고 `record_gate_result`가 기록 시점에 대상을 **다시** 해시하면, verdict와 `target_sha`의 출처가 갈라져 **검토가 끝난 뒤 산출물을 고치고 같은 결과 파일을 재사용하는 replay**(새 내용에 옛 verdict를 붙임)가 성립한다. 세 축으로 결속한다: ① **SubagentStop 훅이 리뷰 종료 시점에 대상의 `review_sha`와 `job_id`를 결과 파일에 각인**한다 ② pilot이 기록 시점 해시·job과 대조해 **불일치·부재면 거부**한다(재검증 요구) ③ 기록에 성공한 결과 파일은 **소비(unlink)** 해 1결과=1기록을 강제한다. 이 설계의 요체는 **해시의 생산자가 리뷰어가 아니라 훅**이라는 점이다 — LLM은 sha256을 신뢰 계산할 수 없지만 훅은 코드다. (진행 원장이 "리뷰어가 SHA를 반환해야 하므로 Read-전용 LLM으로는 비현실적"이라며 P3/P-GUI로 이연했던 Codex 2026-07-14 #4는 이 잘못된 제약 위에 선 판단이었고, 여기서 해소된다.)
...
| PreToolUse | 에이전트의 `pilot approve`(승인·반려·override)·G2 기록 호출 | **무조건 차단** — 승인은 인간 전용 채널(§6.1), G2 기록은 SubagentStop 훅 전용(§4.2). ※ gate-run(G1·G3)은 계산형이라 차단 불요(§4.4). 명령 문자열은 **셸 인용을 제거해 토큰화**한 뒤 검사한다(C-1) — 정규식만 쓰면 `gate-run "G2"` 같은 인용 변형이 패턴을 피하면서 allowlist 접두사는 유지해 **사람 확인 프롬프트조차 뜨지 않는 무성 통과**가 된다. 반대로 경로 인자 추출은 인용 보존 토큰화라야 한다(posix 모드가 Windows 백슬래시를 이스케이프로 소비) |
...
| SubagentStop | gate2-reviewer 완료 | 결과 파싱 → **대상의 `review_sha`·`job_id` 각인**(검토 시점 결속 — 훅이 해시 생산자) → G2 기록(`gate-run G2 --from-subagent <결과파일>`, pilot이 기록 시점 해시와 대조) → **결과 파일 소비(unlink)**. 메인 에이전트 미경유(§4.2, C-1) |
...
- **Phase 1 신뢰 경계**: 승인의 진본성은 "같은 PC, 같은 사용자 계정"(단일 사용자 파일럿)에 의존한다. 다중 사용자 운영으로 확장 시 승인자 인증 채널이 필요하다(§15). **GUI 헤드(P-GUI)도 동일 경계** — 승인 진본성 = OS 로그인 세션 위임: GUI의 `ApprovalExecutor`가 세션 마커(`CLAUDECODE`) 부재 일반 셸 프로세스로 `pilot approve/intervene`를 실행하고, `assert_human_channel`이 마커 감지 시 **거부하되 세탁하지 않는다**(env-strip은 훅 2차 잠금의 인용-우회 백스톱을 리포 내 재사용 세탁도구로 만들어 간접 호출에 뚫리므로 — 거부만). 실행 성공/부분/미확정은 **원장 baseline 앵커**로 확정한다 — 호출 직전 커밋 라인 수(baseline) 이후 커밋된 일치 이벤트만 '적용됨'이며, 절대 상태(현재 rejected·기존 overrides[])로 판정하지 않는다(재반려·동일 sha 재override에서 과거 상태를 이번 성공으로 오인 차단). override 확정은 (gate·target·reason)의 baseline 앵커로 충족되어 `target_sha` 대조를 앵커가 포섭하고, `new` dir-부재는 fail-closed `INDETERMINATE`(rebuild-first 실패)로 판정한다. 비정상 종료(timeout∪exit≠0)는 `pilot rebuild`+재조회, rebuild 실패 시 미확정 유지. **[v1.12] 마일스톤 승인·반려는 pending-only**: `cmd_approve`가 원장 기반으로 "현재 요청·미해소" 마일스톤만 수용한다(§6.1 검토 체크포인트의 선승인 우회 봉쇄 — approve·reject 공통, override 미접촉, advance와 판정 규칙 공유·fold/reader와 3자 일치). GUI 패널 콤보도 `pending_milestone`만 제시(이중 방어)하며, "보는 job = 액션 대상 job" 결속(전환 시 위젯 클리어·대상 선고정·불일치 스냅샷 무시·채팅 세션대상 라벨)이 승인 오타겟을 구조적으로 차단한다.
...
├─ cae_gui/                   # GUI 헤드(D7·P-GUI): PySide6 + claude-agent-sdk, optional 의존 `pip install -e .[gui]`
│  ├─ config.py session.py    # 리포루트·프로파일·게이트웨이 env / build_options(SDK 옵션)·SessionBridge(워커스레드+can_use_tool 브리지)
│  ├─ readmodel/              # reader.py(JobSnapshot·§3.5 tail-read) watcher.py(QFileSystemWatcher+폴링) — 상태 무변조 조회층
│  ├─ approval.py             # ApprovalExecutor(assert_human_channel 마커 fail-closed·build_cmd·baseline 앵커 판정)
│  ├─ widgets/                # main_window chat_view board gate_viewer approval_panel new_job_form
│  └─ smoke/                  # sdk_equivalence.py (SDK/CLI 하네스 동등성 착수 게이트)
│                             # read-model 순수성: jobs/**·pilot/runtime/** 무직접쓰기 — 상태변경은 subprocess pilot·SDK 세션(훅 게이팅)뿐
├─ harness/
...
- **벤더 브리지(hm_bridge) 신뢰 경계(P2.5 하드닝/P3 배치, Codex 적대 리뷰 2026-07-11)**: byte-frozen 상류 fork인 TCP 브리지는 0.0.0.0 무인증 바인딩·`execute_tcl_raw` dispatch 등록·arg 주입 가능한 `execute_tcl_safe`를 그대로 안고 있다. Phase 1 신뢰 경계는 **엔지니어 로컬 HM 콘솔 + localhost relay**(§6.1 단일 사용자 계정)라 즉시 위험은 낮으나, 공유망 배치 전 fork 하드닝 필수 — loopback 기본화·dispatch allowlist(raw 제외)·typed 인자 검증. 이 하드닝은 브리지 실 hwx 구현과 **동일 fork 이벤트(P2.5)**로 묶는다(§13·§15 백로그). MCP 계층은 이미 `execute_tcl_raw`를 노출하지 않으나 TCP dispatch는 별도 차단이 없다. **인자 주입(#1)은 MCP 계층에서 선착지했다(whole-tree 리뷰 2026-07-15 C-2)**: 브리지가 byte-frozen이라 상류 밖에서는 고칠 수 없으므로, 완화를 우리 계층(`cae_tools/hm_mcp/tools/tcl.py`)에 두어 `tcl_args`를 화이트리스트(영숫자·공백·`_-.,=:/@`)로 검증하고 TCL 메타문자를 거부한다 — 브리지의 **명령 접두사 화이트리스트는 보안 경계가 아니다**(인자를 그대로 TCL로 평가하므로 `;`·`[]`·`` ` `` 주입이 접두사를 통과한 뒤 임의 명령이 된다). 이는 **도달 경로를 먼저 닫은 것이지 결함을 고친 것이 아니다** — MCP를 우회하는 TCP dispatch 직결이 남아 있으므로 브리지 본체의 typed 검증은 P2.5로 유지된다(공유망 배치 전 필수).
- **GUI 헤드(P-GUI) 정책 정합 — 해소(착수 게이트 H, 2026-07-18)**: GUI 헤드는 ClaudeApp식 전면 default-DENY를 **채택하지 않는다**. 리포 `settings.json` allowlist가 정책 SSOT(`setting_sources=["project"]`로 상속 — 헌법·훅·skills·`.mcp.json`(hm_mcp) 동반 상속, `strict_mcp_config` 미활성)이고, GUI의 `can_use_tool` 프롬프트는 allowlist 밖 요청에 대한 **사람 확인의 GUI 구현**(CLI 프롬프트와 동형 — 헤드가 하네스 의미론을 바꾸지 않음, D7). 스코핑 거동(allowlist 도구의 무프롬프트 통과)은 착수 게이트 시나리오 H가 **실측 확정**했다(필요 시 브리지가 allowlist를 기계 미러링해 자동 허용하되 정책 원천은 settings.json 유지 — 정책 저작이 아니라 하네스 설정의 반영). 운영(ops) 프로파일의 §10.2 웹 deny(WebFetch·WebSearch)는 프로파일 조건부라 정적 settings.json이 표현 못 하므로 `build_options`의 `disallowed_tools`가 담되 도구 allowlist SSOT는 settings.json 유지. ops 게이트웨이 인증은 두 변수(`ANTHROPIC_BASE_URL`·`ANTHROPIC_AUTH_TOKEN`) 필수의 fail-fast(부재/빈값→`ValueError`, ambient `ANTHROPIC_API_KEY` 무음 폴백 봉쇄 — Codex 적대리뷰 F3). ClaudeApp의 default-DENY는 하네스 OFF 격리 앱의 정책이었다.
...
| SDK/CLI 하네스 동등성 (GUI 헤드) — **해소(P-GUI Task 2 착수 게이트, 2026-07-18)** | P-GUI | ClaudeApp은 격리 목적으로 `setting_sources=[]`로 하네스 로딩을 껐음 — GUI 헤드는 project 로딩을 켜야 하며, hooks 발화·Skills 로딩·승인 채널 행동의 SDK/CLI 동등성을 P-GUI 착수 스모크(§11.2 우회 스위트 재실행 포함)로 실증. **→ 결과: 실 SDK 세션 3회차 종합 A~J 전 항목 동등**(hooks 차단·G1 자동발화·G2 기록 진본성·Skills 로딩·**승인 불변식 동시 성립**[패널 approve 통과 ∧ 세션 내 에이전트 approve 차단]·allowlist 정합) → 폴백(아키텍처 재검토) 불요 확정. **SDK 0.2.120 표면이 CLI와 다른 3지점 실측**: ① 훅 `additionalContext`(게이트 피드백) 스트림 비노출 → **GUI는 게이트 상태를 채팅 스트림 아닌 read-model에서 렌더**(P-GUI read-model 중심 아키텍처 정당화) ② read-only 명령 CLI·SDK 공통 자동허용 ③ 서브에이전트 스폰 도구명 `Agent`(CLI `Task`). Chunk 2~3 최종 whole-branch 리뷰 Ready to merge(658 passed, read-model 순수성·마커 refuse-not-wash 승인 채널·baseline 앵커 판정 실코드 확증) |
...
| 벤더 브리지(hm_bridge) 보안 자세 (Codex 적대 리뷰 2026-07-11 #1·#2·#3) | 모델 변조·중복 실행 | byte-frozen 상류 fork라 P1 미개변. **P2.5 하드닝 대상(실 구현과 동일 fork 이벤트)**: ① `execute_tcl_safe`가 `tcl_args`를 raw 연결 → `; ` 주입으로 화이트리스트 무력화(#1) → 명령별 typed 검증·이스케이프 ② TCP dispatch가 0.0.0.0 무인증 바인딩 + `execute_tcl_raw` 등록(MCP는 숨김·`VENDORED_FROM.md` "노출금지" 명시하나 TCP는 노출, #2) → loopback 기본화·dispatch allowlist ③ 클라이언트 60초 vs 브리지 300초 타임아웃 불일치·미취소로 변형 RPC 재시도 시 중복 실행(#3, 현재 stub이라 잠재—실 hwx 구현 시 발현) → request-id dedup·타임아웃 계약 일치. **실 구현은 MANUAL_P1 실기 게이트 이후**. — **①은 MCP 계층에서 선착지(whole-tree 리뷰 2026-07-15 C-2, `6f04d7e`)**: `hm_mcp`가 `tcl_args`를 화이트리스트로 검증해 **우리 경로의 주입은 차단**됐다(§10.2). 브리지 본체는 무변경이고 TCP dispatch 직결이 남으므로 ①의 typed 검증은 P2.5 유지 — P2.5는 "공격면이 열려 있다"가 아니라 "MCP 밖 경로가 남았다"를 전제로 계획할 것 |
